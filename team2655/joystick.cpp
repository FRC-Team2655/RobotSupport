...
template <class TYPE>
bool team2655::js::polyfit(const std::vector<TYPE> & x, const std::vector<TYPE> & y, const int &order, std::vector<TYPE> &coeffs) {
...
team2655::js::coefficients team2655::js::getCubicCoefficients(double deadband, double minPower, double midPower){
	...
	if(polyfit<double>(xCoords, yCoords, 3, results)){
		...
		// Default to a linear function that will only apply the JS deadband.
		getDeadbandCoefficients(deadband);
	}
	return coefficients;
}

team2655::js::coefficients team2655::js::getDeadbandCoefficients(double deadband){
	deadband = fabs(deadband);
	return std::array<double, 5>{ 0, 1, 0, 0, deadband };
}

double team2655::js::getScaledValue(const coefficients coeffs, const double axisValue, bool deadbandOnly){

	// Adhere to the set deadband
	if(fabs(axisValue) < coeffs[4]){
		return 0;
	}
	...
	if(deadbandOnly || (coeffs[0] == 0 && coeffs[1] == 1 && coeffs[2] == 0 && coeffs[3] == 0)){
		...
		return (axisValue - (fabs(axisValue) / axisValue * coeffs[4])) / (1 - coeffs[4]); // This will do the scaling
	}else{
		// Do everything in the first quadrant (+x, +y) then move to third quadrant if x is (-)
		double x = fabs(axisValue);

		// Do the calculation
		double result = coeffs[3] * pow(x, 3) + coeffs[2] * pow(x, 2) + coeffs[1] * x + coeffs[0];
