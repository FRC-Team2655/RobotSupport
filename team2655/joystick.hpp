...
namespace team2655{
	namespace js{ // The js namespace contains helper functions for use with WPILib Joysticks.

		/**
		 * A type for all the needed coefficients for joysticks
		 */
		typedef std::array<double, 5> coefficients;
		 ...
		 */
		coefficients getCubicCoefficients(double deadband, double minPower, double midPower);
		 ...
		 */
		coefficients getDeadbandCoefficients(double deadband);
		 ...
		 */
		double getScaledValue(const coefficients coeffs, const double axisValue, bool deadbandOnly = false);
	}
}
